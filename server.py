...
def _load_available_skills() -> list[dict]:
    """Read hermes' own skills metadata cache so the admin UI can show a
    pickable list. The snapshot is generated by hermes when building the
    system prompt; if the gateway has never started, the file won't exist
    and we return an empty list (UI shows a friendly message)."""
    snapshot_path = Path(HERMES_HOME) / ".skills_prompt_snapshot.json"
...
async def api_available_skills(request: Request):
    if err := guard(request): return err
    return JSONResponse({"skills": _load_available_skills()})
...
routes = [
    ...
    Route("/api/available-skills",      api_available_skills),
